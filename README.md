...
- **浏览与搜索**：支持分类浏览与关键词搜索。中文关键词可直接搜标题、作者、词牌和**诗词正文**（输入简体自动匹配繁体正文）；也支持拼音搜索（如 `libai`、`wang wei`，连续输入会自动按音节切分），覆盖标题、作者、词牌。记不全整句时（如与底本异文）会做容错匹配，例如搜「床前明月光」可以找到《全唐诗》本的《静夜思》。
- **AI 赏析**：详情页一键生成词汇注释、内容赏析、艺术特色与情感表达，并对正文难词做高亮提示。支持任意 OpenAI 兼容接口（DeepSeek、OpenAI、本地大模型等）。
  - 赏析结果按诗缓存，同一首诗只请求一次，可点击右上角刷新按钮强制重新赏析；
  - 支持填写补充要求（如“侧重分析用典与格律”）后再重新赏析，模型会优先满足你的要求；
