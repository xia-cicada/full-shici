...
-- 创建触发器保持数据同步
CREATE TRIGGER IF NOT EXISTS poetry_ai AFTER INSERT ON poetry
BEGIN
    INSERT INTO poetry_search(
        rowid, title, author, rhythmic, 
        title_pinyin, author_pinyin, rhythmic_pinyin,
        title_initials, author_initials, rhythmic_initials
    ) VALUES (
        new.id, new.title, new.author, new.rhythmic,
        new.title_pinyin, new.author_pinyin, new.rhythmic_pinyin,
        new.title_initials, new.author_initials, new.rhythmic_initials
    );
END;

CREATE TRIGGER IF NOT EXISTS poetry_ad AFTER DELETE ON poetry
BEGIN
    DELETE FROM poetry_search WHERE rowid = old.id;
END;

CREATE TRIGGER IF NOT EXISTS poetry_au AFTER UPDATE ON poetry
BEGIN
    DELETE FROM poetry_search WHERE rowid = old.id;
    INSERT INTO poetry_search(
        rowid, title, author, rhythmic, 
...
-- 初始化基本分类数据（带拼音）
INSERT OR IGNORE INTO categories (id, name, name_pinyin, name_initials, description) VALUES 
  (1, '唐诗/宋诗', 'tang shi song shi', 'tsss', '全唐诗 & 全宋诗'),
  (2, '宋词', 'song ci', 'sc', '全宋词');